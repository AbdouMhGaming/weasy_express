...
export default function QueueView() {
  ...
  async function deleteTicket(ticket: Ticket) {
    if (!confirm(`Supprimer le ticket ${ticket.ticket_ref} ? Cette action est irréversible.`)) return;
    try {
      await fetch(`${API_BASE}/api/tickets/${ticket.id}`, { method: "DELETE", headers: adminHeaders() });
      setSelected(null);
      fetchTickets();
    } catch { }
  }
          ...
          <div className="fixed inset-0 z-50 flex items-stretch justify-end">
            ...
            <div className="relative w-full max-w-lg bg-[#f7f8fa] shadow-2xl flex flex-col z-10" onClick={e => e.stopPropagation()}>
              ...
              <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-2 shrink-0">
                ...
                {isAdmin && (
                  <button onClick={() => deleteTicket(selected)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-500 transition-colors shrink-0" title="Supprimer le ticket">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
